...
function findFrom(board, word, y, x, seen) {
    ...
    if (y > 0 && this.findFrom(board, word.slice(1), y - 1, x, seen)) {
        return true;
    }

    if (y < 4 && this.findFrom(board, word.slice(1), y + 1, x, seen)) {
        return true;
    }

    if (x > 0 && this.findFrom(board, word.slice(1), y, x - 1, seen)) {
        return true;
    }

    if (x < 4 && this.findFrom(board, word.slice(1), y, x + 1, seen)) {
        return true;
    }

    if (y > 0 && x > 0 && this.findFrom(board, word.slice(1), y - 1, x - 1, seen)) {
        return true;
    }

    if (y < 4 && x < 4 && this.findFrom(board, word.slice(1), y + 1, x + 1, seen)) {
        return true;
    }

    if (x > 0 && y < 4 && this.findFrom(board, word.slice(1), y + 1, x - 1, seen)) {
        return true;
    }

    if (x < 4 && y > 0 && this.findFrom(board, word.slice(1), y - 1, x + 1, seen)) {
        return true;
    }
  ...
  /** Can word be found in board? */
    for (let y = 0; y < board.length; y++) {
        for (let x = 0; x < board[y].length; x++) {
            if (this.findFrom(board, word, y, x, new Set())) {
                return true;
            }
        }
    }
